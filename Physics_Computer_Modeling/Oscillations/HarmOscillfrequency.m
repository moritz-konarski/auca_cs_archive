...
%dialog bos asking user for frequency
winput = inputdlg ('Type in Frequency (number)','Oscillation');         %getting user imput for frequency
w=str2double(winput);                                                   %frequency converted from user input

%-----------preparing window----------------------------
figure('units','norm','position',[.1 .1 .8 .7],'color',[.9 .9 .9],...
       'name', "Harmonic Oscillation Frequency");                       %Creating Window
a = axes('position',[.1 .1 .8 .8],'color',[1 1 1],'fontsize',[12]);     %Creating Axis
txt= [ "Frequency Modulation\ny=sin(" num2str(w) "*x)"];                          %preparing frequency string
