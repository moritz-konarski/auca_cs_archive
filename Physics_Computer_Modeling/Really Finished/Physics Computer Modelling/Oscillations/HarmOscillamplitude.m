...
Ainput = inputdlg ('Type in Amplitude (number)','Oscillation');         %getting user imput for Amplitude
A=str2double(Ainput);                                                  %amplitude converted from user input

%-----------preparing window----------------------------
figure('units','norm','position',[.1 .1 .8 .7],'color',[.9 .9 .9],...
       'name', "Harmonic Oscillation Amplitude");                       %Creating Window
a = axes('position',[.1 .1 .8 .8],'color',[1 1 1],'fontsize',[12]);      %Creating Axis
txt= [ "Amplitude Modulation\ny=" num2str(A) "*sin(x)"];                  %preparing amplitude string
