%Topic: Newtons 2nd Law, exemplified by projectile

%=====dialog box asking user for input========
input = inputdlg ({"Starting Velocity v0"},...
                   "Projectile Motion",[1,20],{"1000"});           %getting user imput
vx=v0=str2num(input{1});                                                           %velocity converted from user input

%=====preparing window========================
figure('units','norm','position',[.1 .1 .8 .7],'color',[.9 .9 .9],...
       'name', "Newtons 2nd Law");                                              %Creating Window
a = axes('position',[.1 .1 .8 .8],'color',[1 1 1],'fontsize',[12]);             %Creating Axis
txt = ["Newtons 2nd Law - Satellite\nv0=" num2str(v0) "m/s"];                  %preparing  string
a : title (txt,'fontsize',[17]);                                                %titleing diagram
a : xlabel ("X in 10^3 km",'fontsize',[17]);                                         %labeling y axis
a : ylabel ("Y in 10^3 km",'fontsize',[17]);                                         %labeling x axis
grid on;                                                                        %turing on the grid in coordinate system
drawnow;                                                                        %drawing the system
axis('equal',[-10^7 10^7 -10^7 10^7]);                %setting axis limits

x=x0=vy=0;
y=y0=7.4*10^6;
k=1;
dt=20;

%=====drawing earth===========================
for fi=0:.01:2*pi;                            
X_earth(k)=6.4*10^6*cos(fi);
Y_earth(k)=6.4*10^6*sin(fi);
k=k+1;
endfor;
h_earth=line(X_earth,Y_earth,'linewidth',[5],'color','blue');                   %drawing earth

%=====declaring function for acceleration x===
function ax=funVX(x,y);
  Me=5.97.*10.^(24);
  G=6.67.*10.^(-11);                 %product of G and Me
  Rs=sqrt(x.^2+y.^2);                               %setting satellite radius
  f1=G*Me./Rs.^2;
  f2=-x*f1./Rs;
  ax=f2;
  %ax=-x.*GMe./(Rs.^3);
endfunction;

%=====declaring function for velocity x=======
function Vx=funX(vx);
  Vx=vx;
endfunction;

%=====declaring function for acceleration y===
function ay=funVY(x,y);
  Me=5.97.*10.^(24)
  G=6.67.*10.^(-11);                 %product of G and Me
  Rs=sqrt(x.^2+y.^2);                               %setting satellite radius
  f1=G*Me./Rs.^2;
  f2=-y*f1./Rs;
  ay=f2;
  %ay=-y.*GMe./(Rs.^3);
endfunction;

%=====declaring function for velocity y=======
function Vy=funY(vy);
  Vy=vy;                  
endfunction;

%=====loop for time progression and graphing==
for t=0:dt:10000;
  
  %=====X + Y variable========================
  %---calculating k1---
  k1x=funX(vx);
  k1y=funY(vy);
  k1Vx=funVX(x,y);
  k1Vy=funVY(x,y);
  
  %---calculating k2---
  k2x=funX(vx+k1Vx*dt/2);
  k2y=funY(vy+k1Vy*dt/2);
  k2Vx=funVX(x+k1x*dt/2,y+k1y*dt/2);
  k2Vy=funVY(x+k1x*dt/2,y+k1y*dt/2);
  
  %---calculating k3---
  k3x=funX(vx+k2Vx*dt/2);
  k3y=funY(vy+k2Vy*dt/2);
  k3Vx=funVX(x+k2x*dt/2,y+k2y*dt/2);
  k3Vy=funVY(x+k2x*dt/2,y+k2y*dt/2);
  
  %---calculating k4---
  k4x=funX(vx+k3Vx*dt);
  k4y=funY(vy+k3Vy*dt);
  k4Vx=funVX(x+k3x*dt,y+k3y*dt);
  k4Vy=funVY(x+k3x*dt,y+k3y*dt);
  
  %---calculating new x & y---
  x=x+(k1x+2*k2x+2*k3x+k4x)*dt/600;
  y=y+(k1y+2*k2y+2*k3y+k4y)*dt/6;
  
  %---calculating new vx & vy---
  vx=vx+(k1Vx+2*k2Vx+2*k3Vx+k4Vx)*dt/6;
  vy=vy+(k1Vy+2*k2Vy+2*k3Vy+k4Vy)*dt/6;

  %===creating line of y and x================
  line([x0  x],[y0 y],'linewidth',[2]);        %line of x and y
  x0=x;
  y0=y; 
  drawnow;
  
  %===Breack if projectile hits the ground====
  if sqrt(x.^2+y.^2)<6.4*10^6;
    break;
  endif;
  %===Breack if projectile hits the ground====
  if sqrt(x.^2+y.^2)>10^7;
    break;
  endif;
    
endfor;