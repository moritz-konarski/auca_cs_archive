%Topic: Newtons 2nd Law, exemplified by lorentz-force and strenght of field in magnetic field

%=====dialog box asking user for input========
input = inputdlg ({"Enter the values for the simulation\nvx", "\nB","\nm","\nE","\nQ"},"Object in Magnetic Field",...
                   [1,10;1,10;1,10;1,10;1,10],{"1","1","1","1","1"});           %getting user imput
...
vx=vx1;
x=x0=0;                                       %initializing
y=y0=z0=vz=vy=z=0;                            %initializing
dt=.02;                                       %interval so that one full circle has 500 steps
limt=1000;                                    %setting limit of loop to stop after one full radius
...
txt = ["Newtons 2nd Law - Magnetic Field\nvx=" num2str(vx1) " ; B=" num2str(B0) ...
       " ; m=" num2str(m) "\nE=" num2str(E0) " ; Q=" num2str(Q) ];              %preparing  title, stating variables
...
axis('equal',[-1 20 -1 10 -1 10]);                                              %setting axis limits based on radius of circle ,[-1 10 -5 5 -5 5]
h_center=line(0,0,'marker','o', 'markersize',[10],'color','red',...
...
function ax=funVX(x,y,vx,vy,B,Q,m,E);
...
function ay=funVY(x,y,vx,vy,vz,B,Q,m,E);
  ay=(Q*(E))/(m);                             %acceleration for y +vy*B
endfunction;
...
function az=funVZ(x,y,vx,vy,vz,B,Q,m,E);
  az=(Q*(vx*B))/(m);                          %acceleration for z E+
endfunction;
...
for t=0:dt:limt;                              %loop from 0 to limt with interval dt
  ...
  set(h_center,'xdata',x,'ydata',y,'zdata',z);  %setting the ball to the center
