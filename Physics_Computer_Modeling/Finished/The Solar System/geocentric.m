...
%------------preparation-----------------------------------------------------------------------------------------
figure('units','norm','position',[.1 .1 .8 .7],'color',[.9 .9 .9],'name', "Geocentric"); %Creating Window
axes('position',[.1 .1 .8 .8],'color',[1 1 1], 'xlim',[-18 18],'ylim',[-18 18]);  %Creating Axis
...
%----To adapt this system, set ws and rs for the sun to the ones of the desired planet
...
%------------Drawing Sun------------------------------------------------------------------------------------------
h_Sun=line(0,0,0,'marker','o','markersize',[18],'markerfacecolor','yellow','color','yellow'); %creating the ball
ws=1;               %angular speed
rs=5;               %orbit radius
...
%------------Drawing Venus----------------------------------------------------------------------------------------
h_Venus=line(0,0,0,'marker','o','markersize',[8],'color',[0 .7 0],'markerfacecolor',[0 .7 0]); %creating the ball
...
for t=0:i:l                                          %setting up loop for one rotation
  ...
  if (mod(t,0.5.*i)==g)     %Trails of Planets
    ...
    %line(xem-xs,yem-ys,'marker','.','markersize',[7]); %Moon
    line(xma-xs,yma-ys,'marker','.','markersize',[7]);   %Mars
    %line(xmam-xs,ymam-ys,'marker','.','markersize',[7]); %Marsmoon
    line(xj-xs,yj-ys,'marker','.','markersize',[7]);   %Jupiter
    %line(xmj1-xs,ymj1-ys,'marker','.','markersize',[7]); %Jupiter Moon one
    %line(xmj2-xs,ymj2-ys,'marker','.','markersize',[7]); %Jupiter Moon two
    %line(xmj3-xs,ymj3-ys,'marker','.','markersize',[7]); %Jupiter Moon three
    %line(xmj4-xs,ymj4-ys,'marker','.','markersize',[7]); %Jupiter Moon four
