...
int main(int argc, char **argv, char **envp) {
    ...
    char *home = ish_get_first_environment_variable(
    ...
    //    make string for input and terminate it correctly
    ...
    while (read(0, input, Max_Input_String_Length) >= 0) {
        ...
        ish_replace_first_character_in_cstring(input, '\n', '\0');
        ...
        unsigned long argument_count = ish_build_argument_array_from_input(
                input,
                arguments,
                Max_Argument_Count);
        ...
        //    separate process.
        if (ish_are_cstrings_equal(command, Builtin_Command_CD)) {
        ...
        if (ish_are_cstrings_equal(command, Builtin_Command_Exit)) {
            int exit_status = argument_count == 0 ? 0 :
                              ish_get_integer_from_cstring(arguments[1]);
        ...
        //    `man 2 open`
        int stdin_descriptor = !stdin_file ? -1 : open(stdin_file,
                                                       Standard_Input_File_Flags);

        //    Search for the stdout redirection lexeme in the list
        //    of arguments.
        char *stdout_file = ish_extract_last_stdout_redirection_file(
                arguments);
        ...
        int stdout_descriptor = !stdout_file ? -1 : creat(stdout_file,
