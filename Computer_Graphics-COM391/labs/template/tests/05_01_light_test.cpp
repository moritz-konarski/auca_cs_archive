...
int main(int argc, char **argv) {
    ...
    auto phong_plane = generate_rectangle_mesh(LENGTH_PLANE_SIDE, LENGTH_PLANE_SIDE, "phong plane",
                                               POSITION_PLANE, COLOR_PLANE, VERTEX_COUNT_PLANE,
    ...
    for (;;) {
        ...
        point_light1->set_position(
                glm::vec3(RADIUS_LIGHT_1 * cos(angle_light_1), RADIUS_LIGHT_1 * sin(angle_light_1),
                          Z_VALUE_LIGHTS));
        angle_light_1 += ROTATION_SPEED_LIGHT_1;
        point_light2->set_position(
                glm::vec3(RADIUS_LIGHT_2 * cos(angle_light_2), RADIUS_LIGHT_2 * sin(angle_light_2),
                          Z_VALUE_LIGHTS));
        angle_light_2 += ROTATION_SPEED_LIGHT_2;
        point_light3->set_position(
                glm::vec3(RADIUS_LIGHT_3 * cos(angle_light_3), RADIUS_LIGHT_3 * sin(angle_light_3),
                          Z_VALUE_LIGHTS));
