...
int main(int argc, char **argv) {
    ...
    point_light_red->set_ambient_color(COLOR_RED);
    ...
    auto point_light_green = generate_point_light(scene, lamp_base_green,
                                                  INTENSITY_LIGHT);
    point_light_green->set_ambient_color(COLOR_GREEN);
    ...
    point_light_blue->set_ambient_color(COLOR_BLUE);
    point_light_blue->set_position(
            generate_random_glm_vec3(-CUBE_BOUNDARY + RADIUS_LIGHT,
                                     CUBE_BOUNDARY - RADIUS_LIGHT));
    glm::vec3 point_light_blue_direction = generate_random_glm_vec3(MIN_SPEED, MAX_SPEED);

    auto point_light_magenta = generate_point_light(scene, lamp_base_magenta,
                                                    INTENSITY_LIGHT);
    point_light_magenta->set_ambient_color(COLOR_MAGENTA);
