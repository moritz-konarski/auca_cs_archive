    ...
    namespace mesh_group_generators {
        ...
        void generate_circle_of_circle_meshes(std::shared_ptr<Mesh> parent_object,
            ...
            for (int i = 1; i <= circle_count; ++i) {
                if (i % modulus_value != 0) {
                    angle = angle_ratio * i;
                    auto circle = std::make_shared<Mesh>(circle_base_geometry,
                                                         circle_base_material,
                                                         glm::vec3{circle_radius *
                                                                   cos(angle),
                                                                   circle_radius *
                                                                   sin(angle),
                                                                   z_offset});
        ...
        void generate_ring_of_square_meshes(std::shared_ptr<Mesh> parent_object,
            ...
            for (int i = 1; i <= square_count; ++i) {
                if (i % modulus_value != 0) {
                    angle = angle_ratio * i;
                    auto square = std::make_shared<Mesh>(square_base_geometry,
                                                         square_base_material,
                                                         glm::vec3{(circle_radius) *
                                                                   cos(angle),
                                                                   (circle_radius) *
                                                                   sin(angle),
