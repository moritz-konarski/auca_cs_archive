...
void main() {
    ...
    for (int i = 0; i < DIRECTIONAL_LIGHT_COUNT; i++) {
        if (directional_light_enabled[i]) {
            float n_dot_l = max(dot(view_normal, directional_light_view_direction[i]),
                                0.0);
            vec3 diffuse_color =
                    material_diffuse_color.rgb * directional_light_diffuse_color[i];
            vec3 diffuse_term = n_dot_l * diffuse_color;

            vec3 reflection_vector = reflect(-directional_light_view_direction[i],
                                             view_normal);
            float n_dot_h = clamp(dot(view_direction, reflection_vector), 0.0, 1.0);
            vec3 specular_color =
                    material_specular_color.rgb * directional_light_specular_color[i];
            vec3 specular_term =
            ...
            if (directional_light_two_sided[i]) {
                ...
                n_dot_l = max(
                        dot(inverted_view_normal, directional_light_view_direction[i]),
                        0.0);
