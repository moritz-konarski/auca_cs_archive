/*
 * first defines many variables based on which lights are selected
 * then computes all the values for each of then in a loop for each fragment
 */
...
uniform vec3
directional_light_view_direction[DIRECTIONAL_LIGHT_COUNT];
uniform vec3
directional_light_ambient_color[DIRECTIONAL_LIGHT_COUNT];
uniform vec3
directional_light_diffuse_color[DIRECTIONAL_LIGHT_COUNT];
uniform vec3
directional_light_specular_color[DIRECTIONAL_LIGHT_COUNT];
...
uniform vec3
point_light_view_position[POINT_LIGHT_COUNT];
uniform vec3
point_light_ambient_color[POINT_LIGHT_COUNT];
uniform vec3
point_light_diffuse_color[POINT_LIGHT_COUNT];
uniform vec3
point_light_specular_color[POINT_LIGHT_COUNT];
...
uniform vec3
spot_light_view_position[SPOT_LIGHT_COUNT];
uniform vec3
spot_light_view_direction[SPOT_LIGHT_COUNT];
uniform vec3
spot_light_ambient_color[SPOT_LIGHT_COUNT];
uniform vec3
spot_light_diffuse_color[SPOT_LIGHT_COUNT];
uniform vec3
spot_light_specular_color[SPOT_LIGHT_COUNT];
...
void main() {
    ...
    for (int i = 0; i < DIRECTIONAL_LIGHT_COUNT; i++) {
        if (directional_light_enabled[i]) {
            ...
            front_color.rgb += directional_light_intensity[i] *
                               (directional_light_ambient_color[i] + diffuse_term + specular_term);
            ...
            if (directional_light_two_sided[i]) {
                ...
                reflection_vector = reflect(-directional_light_view_direction[i],
    ...
    for (int i = 0; i < POINT_LIGHT_COUNT; i++) {
        if (point_light_enabled[i]) {
            ...
            front_color.rgb += attenuation_factor *
                               (point_light_ambient_color[i] + diffuse_term + specular_term);
    ...
    for (int i = 0; i < SPOT_LIGHT_COUNT; ++i) {
        if (spot_light_enabled[i]) {
            ...
            front_color.rgb += attenuation_factor *
                               (spot_light_ambient_color[i] + diffus_term + specular_term);
