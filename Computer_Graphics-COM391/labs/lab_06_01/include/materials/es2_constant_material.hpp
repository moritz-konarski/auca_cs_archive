/*
 * child class of constant material and thus constant material
 * specifies shader positions and attributes
 * sends the updated information to the shader
 */
...
namespace aur {
    ...
    public:
        ES2ConstantMaterial() {
            std::string vertex_shader_source = FileUtilities::read_text_file(
                    "../data/shaders/es2_constant_shader.vert");
            std::string fragment_shader_source = FileUtilities::read_text_file(
            ...
            _shader = std::make_shared<ES2Shader>(vertex_shader_source, fragment_shader_source,
                                                  attributes, uniforms);
        }
        ...
        void update(std::shared_ptr<Scene> scene, std::shared_ptr<Mesh> mesh) final {
            ...
            glm::mat4 mvp_matrix =
                    scene->get_camera()->get_view_projection_matrix() * mesh->get_world_matrix();
