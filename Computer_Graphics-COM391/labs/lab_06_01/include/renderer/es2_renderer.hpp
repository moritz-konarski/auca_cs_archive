/*
 * implements an actual renderer, operates on a queue basis
 * for each element uses, binds, updates and then draws the elements
 * converts the geometry type enum to actual opengl types
 */
    ...
    public:
        ...
        void render() final {
            ...
            if (camera->should_receive_aspect_ratio_from_renderer()) {
                float aspect_ratio = fabsf(static_cast<float>(window->get_width()) /
