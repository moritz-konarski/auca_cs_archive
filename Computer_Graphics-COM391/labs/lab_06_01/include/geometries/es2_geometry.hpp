 ...
 * implements update so that data is only sent when necessary, binds index, vertex buffers, finds what elements
 * of vertex are defined, determines the stride size (amount of data per vertex)
 * implements bind
 */
    ...
    public:
        ...
        void update(const Material &material) final {
            ...
            int position_attribute_location{
                    attributes.count("position") > 0 ? attributes.at("position") : -1};
            ...
            int texture_coordinates_attribute_location{
                    attributes.count("texture_coordinates") > 0 ? attributes.at(
