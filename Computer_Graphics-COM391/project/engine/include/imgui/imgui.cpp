...
static ImVec2
CalcNextScrollFromScrollTargetAndClamp(ImGuiWindow *window, bool snap_on_edges);
...
static void
WindowSettingsHandler_ReadLine(ImGuiContext *, ImGuiSettingsHandler *, void *entry,
                               const char *line);

static void
WindowSettingsHandler_WriteAll(ImGuiContext *, ImGuiSettingsHandler *,
                               ImGuiTextBuffer *buf);
...
// Misc
    ...
    static bool
    UpdateWindowManualResize(ImGuiWindow *window, const ImVec2 &size_auto_fit,
                             int *border_held,
    ...
    static void
    RenderWindowTitleBarContents(ImGuiWindow *window, const ImRect &title_bar_rect,
...
void ImGuiIO::AddInputCharacter(unsigned int c) {
    InputQueueCharacters.push_back(
            c > 0 && c <= IM_UNICODE_CODEPOINT_MAX ? (ImWchar) c
...
static void
BezierClosestPointCasteljauStep(const ImVec2 &p, ImVec2 &p_closest, ImVec2 &p_last,
    ...
    if ((d2 + d3) * (d2 + d3) < tess_tol * (dx * dx + dy * dy)) {
        ...
        BezierClosestPointCasteljauStep(p, p_closest, p_last, p_closest_dist2, x1, y1,
                                        x12, y12,
                                        x123, y123, x1234, y1234, tess_tol, level + 1);
        BezierClosestPointCasteljauStep(p, p_closest, p_last, p_closest_dist2, x1234,
                                        y1234, x234,
...
ImU64 ImFileGetSize(ImFileHandle f) {
    ...
    return ((off = ftell(f)) != -1 && !fseek(f, 0, SEEK_END) && (sz = ftell(f)) != -1 &&
            !fseek(f, off, SEEK_SET)) ? (ImU64) sz : (ImU64) -1;
}
...
void
ImGui::ColorConvertRGBtoHSV(float r, float g, float b, float &out_h, float &out_s,
                            float &out_v) {
    float K = 0.f;
    if (g < b) {
        ImSwap(g, b);
...
void
ImGui::ColorConvertHSVtoRGB(float h, float s, float v, float &out_r, float &out_g,
                            float &out_b) {
    if (s == 0.0f) {
        // gray
        out_r = out_g = out_b = v;
...
int ImGuiStorage::GetInt(ImGuiID key, int default_val) const {
    ImGuiStoragePair *it = LowerBound(const_cast<ImVector<ImGuiStoragePair> &>(Data),
                                      key);
...
void
ImGui::CalcListClipping(int items_count, float items_height, int *out_items_display_start,
...
void ImGui::PopStyleVar(int count) {
    ...
    while (count > 0) {
        ...
        if (info->Type == ImGuiDataType_Float &&
            info->Count == 1) { ((float *) data)[0] = backup.BackupFloat[0]; }
...
ImGui::RenderText(ImVec2 pos, const char *text, const char *text_end,
    ...
    if (text != text_display_end) {
        window->DrawList->AddText(g.Font, g.FontSize, pos, GetColorU32(ImGuiCol_Text),
                                  text,
                                  text_display_end);
        if (g.LogEnabled)
...
void ImGui::RenderTextClippedEx(ImDrawList *draw_list, const ImVec2 &pos_min,
    ...
    ImVec2 pos = pos_min;
    const ImVec2 text_size = text_size_if_known ? *text_size_if_known : CalcTextSize(text,
                                                                                     text_display_end,
                                                                                     false,
                                                                                     0.0f);
...
void
ImGui::RenderTextClipped(const ImVec2 &pos_min, const ImVec2 &pos_max, const char *text,
...
void ImGui::RenderTextEllipsis(ImDrawList *draw_list, const ImVec2 &pos_min,
    ...
    if (text_size.x > pos_max.x - pos_min.x) {
        ...
        const float text_avail_width = ImMax(
                ...
                1.0f);
        float text_size_clipped_x = font->CalcTextSizeA(font_size, text_avail_width, 0.0f,
                                                        text,
                                                        text_end_full,
                                                        &text_end_ellipsis).x;
        ...
        while (text_end_ellipsis > text && ImCharIsBlankA(text_end_ellipsis[-1])) {
            ...
            text_size_clipped_x -= font->CalcTextSizeA(font_size, FLT_MAX, 0.0f,
                                                       text_end_ellipsis,
                                                       text_end_ellipsis +
                                                       1).x; // Ascii blanks are always 1 byte
        }
...
ImGui::RenderNavHighlight(const ImRect &bb, ImGuiID id, ImGuiNavHighlightFlags flags) {
    ...
    float rounding = (flags & ImGuiNavHighlightFlags_NoRounding) ? 0.0f
...
void ImGui::UpdateMouseWheel() {
    ...
    if (g.IO.MouseWheel != 0.0f && g.IO.KeyCtrl && g.IO.FontAllowUserScaling) {
        ...
        if (!(window->Flags & ImGuiWindowFlags_ChildWindow)) {
            const ImVec2 offset =
                    window->Size * (1.0f - scale) * (g.IO.MousePos - window->Pos) /
                    window->Size;
    ...
    const float wheel_y = (g.IO.MouseWheel != 0.0f && !g.IO.KeyShift) ? g.IO.MouseWheel
                                                                      : 0.0f;
...
void ImGui::UpdateHoveredWindowAndCaptureFlags() {
    ...
    g.IO.WantTextInput = (g.WantTextInputNextFrame != -1) ? (g.WantTextInputNextFrame !=
                                                             0) : false;
}
...
void ImGui::NewFrame() {
    ...
    g.IO.Framerate = (g.FramerateSecPerFrameAccum > 0.0f) ? (1.0f /
                                                             (g.FramerateSecPerFrameAccum /
                                                              (float) IM_ARRAYSIZE(
    ...
    g.DrawListSharedData.ClipRectFullscreen = ImVec4(0.0f, 0.0f, g.IO.DisplaySize.x,
                                                     g.IO.DisplaySize.y);
...
static void
AddWindowToDrawData(ImVector<ImDrawList *> *out_render_list, ImGuiWindow *window) {
    ImGuiContext &g = *GImGui;
...
static void FindHoveredWindow() {
    ...
    ImVec2 padding_for_resize_from_edges = g.IO.ConfigWindowsResizeFromEdges ? ImMax(
            g.Style.TouchExtraPadding, ImVec2(WINDOWS_RESIZE_FROM_EDGES_HALF_THICKNESS,
...
bool ImGui::IsItemToggledOpen() {
    ImGuiContext &g = *GImGui;
    return (g.CurrentWindow->DC.LastItemStatusFlags & ImGuiItemStatusFlags_ToggledOpen)
           ? true
           : false;
}
...
bool ImGui::IsItemToggledSelection() {
    ...
    return (g.CurrentWindow->DC.LastItemStatusFlags &
            ImGuiItemStatusFlags_ToggledSelection) ? true
                                                   : false;
}
...
bool
ImGui::BeginChildEx(const char *name, ImGuiID id, const ImVec2 &size_arg, bool border,
...
void ImGui::EndChild() {
    ...
    if (window->BeginCount > 1) {
        ...
        if (window->AutoFitChildAxises & (1
...
static ImVec2 CalcWindowSizeAfterConstraint(ImGuiWindow *window, ImVec2 new_size) {
    ...
    if (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSizeConstraint) {
        ...
        new_size.x = (cr.Min.x >= 0 && cr.Max.x >= 0) ? ImClamp(new_size.x, cr.Min.x,
                                                                cr.Max.x)
                                                      : window->SizeFull.x;
        new_size.y = (cr.Min.y >= 0 && cr.Max.y >= 0) ? ImClamp(new_size.y, cr.Min.y,
                                                                cr.Max.y)
                                                      : window->SizeFull.y;
    ...
    if (!(window->Flags &
        ...
        new_size.y = ImMax(new_size.y, window_for_height->TitleBarHeight() +
                                       window_for_height->MenuBarHeight() + ImMax(0.0f,
...
static ImVec2 CalcWindowAutoFitSize(ImGuiWindow *window, const ImVec2 &size_contents) {
    ...
    ImVec2 size_decorations = ImVec2(0.0f,
    ...
    if (window->Flags & ImGuiWindowFlags_Tooltip) {
        ...
        ImVec2 size_auto_fit_after_constraint = CalcWindowSizeAfterConstraint(window,
                                                                              size_auto_fit);
...
static void
CalcResizePosSizeFromAnyCorner(ImGuiWindow *window, const ImVec2 &corner_target,
...
static ImRect
GetResizeBorderRect(ImGuiWindow *window, int border_n, float perp_padding,
                    float thickness) {
...
static bool
ImGui::UpdateWindowManualResize(ImGuiWindow *window, const ImVec2 &size_auto_fit,
                                int *border_held,
                                int resize_grip_count, ImU32 resize_grip_col[4]) {
    ImGuiContext &g = *GImGui;
    ...
    const float grip_hover_outer_size = g.IO.ConfigWindowsResizeFromEdges
                                        ? WINDOWS_RESIZE_FROM_EDGES_HALF_THICKNESS : 0.0f;
    ...
    for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++) {
        ...
        if (resize_grip_n == 0 || held || hovered)
            resize_grip_col[resize_grip_n] = GetColorU32(
                    held ? ImGuiCol_ResizeGripActive : hovered
...
static inline void
ClampWindowRect(ImGuiWindow *window, const ImRect &rect, const ImVec2 &padding) {
    ImGuiContext &g = *GImGui;
    ImVec2 size_for_clamping = (g.IO.ConfigWindowsMoveFromTitleBarOnly &&
                                !(window->Flags & ImGuiWindowFlags_NoTitleBar)) ? ImVec2(
            window->Size.x, window->TitleBarHeight()) : window->Size;
        ...
        static const ImGuiResizeBorderDef resize_border_def[4] =
                {
                        ...
                        {ImVec2(-1, 0), ImVec2(1, 0), ImVec2(1, 1), IM_PI *
                                                                    0.00f}, // Right
                        {ImVec2(0, -1), ImVec2(1, 1), ImVec2(0, 1), IM_PI *
                                                                    0.50f}, // Bottom
                        {ImVec2(+1, 0), ImVec2(0, 1), ImVec2(0, 0), IM_PI *
                                                                    1.00f}  // Left
                };
    ...
    if (window->Collapsed) {
        ...
        if (!(flags & ImGuiWindowFlags_NoResize)) {
            for (int resize_grip_n = 0;
                 resize_grip_n < resize_grip_count; resize_grip_n++) {
                const ImGuiResizeGripDef &grip = resize_grip_def[resize_grip_n];
                const ImVec2 corner = ImLerp(window->Pos, window->Pos + window->Size,
                                             grip.CornerPosN);
                window->DrawList->PathLineTo(
                        corner + grip.InnerDir * ((resize_grip_n & 1) ? ImVec2(
                                window_border_size, resize_grip_draw_size) : ImVec2(
                                resize_grip_draw_size,
                                window_border_size)));
                window->DrawList->PathLineTo(
                        corner + grip.InnerDir * ((resize_grip_n & 1) ? ImVec2(
                                resize_grip_draw_size, window_border_size) : ImVec2(
                                window_border_size,
                                resize_grip_draw_size)));
...
void
ImGui::RenderWindowTitleBarContents(ImGuiWindow *window, const ImRect &title_bar_rect,
                                    const char *name, bool *p_open) {
    ImGuiContext &g = *GImGui;
    ImGuiStyle &style = g.Style;
    ImGuiWindowFlags flags = window->Flags;
    ...
    const float marker_size_x = (flags & ImGuiWindowFlags_UnsavedDocument) ? CalcTextSize(
            UNSAVED_DOCUMENT_MARKER, NULL, false).x : 0.0f;
    ...
    if (flags & ImGuiWindowFlags_UnsavedDocument) {
        ImVec2 marker_pos = ImVec2(ImMax(layout_r.Min.x, layout_r.Min.x +
                                                         (layout_r.GetWidth() -
                                                          text_size.x) *
        ...
        RenderTextClipped(marker_pos + off, layout_r.Max + off, UNSAVED_DOCUMENT_MARKER,
                          NULL, NULL,
...
bool ImGui::Begin(const char *name, bool *p_open, ImGuiWindowFlags flags) {
    ...
    if (window_just_created) {
        ImVec2 size_on_first_use = (g.NextWindowData.Flags &
    ...
    ImGuiWindow *parent_window_in_stack = g.CurrentWindowStack.empty() ? NULL
    ...
    if (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasPos) {
        ...
        if (window_pos_set_by_api &&
            ...
            window->SetWindowPosAllowFlags &= ~(ImGuiCond_Once | ImGuiCond_FirstUseEver |
                                                ImGuiCond_Appearing);
        } else {
    ...
    if (first_begin_of_the_frame) {
        ...
        const bool window_is_child_tooltip =
                (flags & ImGuiWindowFlags_ChildWindow) && (flags &
        ...
        if ((flags & ImGuiWindowFlags_AlwaysAutoResize) && !window->Collapsed) {
            ...
            if (!window_size_x_set_by_api && window->AutoFitFramesX > 0) {
                window->SizeFull.x = window->AutoFitOnlyGrows ? ImMax(window->SizeFull.x,
                                                                      size_auto_fit.x)
                                                              : size_auto_fit.x;
                use_current_size_for_scrollbar_x = true;
            }
            if (!window_size_y_set_by_api && window->AutoFitFramesY > 0) {
                window->SizeFull.y = window->AutoFitOnlyGrows ? ImMax(window->SizeFull.y,
                                                                      size_auto_fit.y)
                                                              : size_auto_fit.y;
                use_current_size_for_scrollbar_y = true;
            }
        ...
        window->Size = window->Collapsed && !(flags & ImGuiWindowFlags_ChildWindow)
                       ? window->TitleBarRect().GetSize() : window->SizeFull;
        ...
        if (window_just_activated_by_user &&
            ...
            if (flags & ImGuiWindowFlags_Popup)
                want_focus = true;
                    ...
                    (flags & (ImGuiWindowFlags_ChildWindow | ImGuiWindowFlags_Tooltip)) ==
                    0)
                want_focus = true;
        }
        ...
        if (!window->Collapsed) {
            ...
            ImVec2 needed_size_from_last_frame = window_just_created ? ImVec2(0, 0) :
                                                 window->ContentSize +
        ...
        window->ScrollMax.x = ImMax(0.0f, window->ContentSize.x +
                                          window->WindowPadding.x * 2.0f -
                                          window->InnerRect.GetWidth());
        window->ScrollMax.y = ImMax(0.0f, window->ContentSize.y +
        ...
        const float work_rect_size_x = (window->ContentSizeExplicit.x != 0.0f
                                        ? window->ContentSizeExplicit.x : ImMax(
        ...
        const float work_rect_size_y = (window->ContentSizeExplicit.y != 0.0f
                                        ? window->ContentSizeExplicit.y : ImMax(
        ...
        window->DC.NavFocusScopeIdCurrent = parent_window
        ...
        */
        ...
        window->DC.LastItemStatusFlags = IsMouseHoveringRect(title_bar_rect.Min,
                                                             title_bar_rect.Max,
                                                             false)
...
void ImGui::BringWindowToFocusFront(ImGuiWindow *window) {
    ...
    if (g.WindowsFocusOrder.back() == window)
        return;
    for (int i = g.WindowsFocusOrder.Size - 2;
         i >= 0; i--) // We can ignore the top-most window
...
void ImGui::PopTextWrapPos() {
    ...
    window->DC.TextWrapPos = window->DC.TextWrapPosStack.empty() ? -1.0f
...
void ImGui::SetWindowPos(ImGuiWindow *window, const ImVec2 &pos, ImGuiCond cond) {
    ...
    IM_ASSERT(cond == 0 || ImIsPowerOfTwo(
...
void ImGui::SetWindowSize(ImGuiWindow *window, const ImVec2 &size, ImGuiCond cond) {
    ...
    IM_ASSERT(cond == 0 || ImIsPowerOfTwo(
            cond)); // Make sure the user doesn't attempt to combine multiple condition flags.
    window->SetWindowSizeAllowFlags &= ~(ImGuiCond_Once | ImGuiCond_FirstUseEver |
                                         ImGuiCond_Appearing);
...
void ImGui::SetWindowCollapsed(ImGuiWindow *window, bool collapsed, ImGuiCond cond) {
    ...
    if (cond && (window->SetWindowCollapsedAllowFlags & cond) == 0)
        return;
    window->SetWindowCollapsedAllowFlags &= ~(ImGuiCond_Once | ImGuiCond_FirstUseEver |
                                              ImGuiCond_Appearing);
...
bool
ImGui::DebugCheckVersionAndDataLayout(const char *version, size_t sz_io, size_t sz_style,
...
static void ImGui::ErrorCheckBeginEndCompareStacksSize(ImGuiWindow *window, bool write) {
    ...
    IM_ASSERT(
            p == window->DC.StackSizesBackup + IM_ARRAYSIZE(window->DC.StackSizesBackup));
}
...
void ImGui::ItemSize(const ImVec2 &size, float text_baseline_y) {
    ...
    const float offset_to_match_baseline_y = (text_baseline_y >= 0) ? ImMax(0.0f,
    ...
    window->DC.PrevLineTextBaseOffset = ImMax(window->DC.CurrLineTextBaseOffset,
                                              text_baseline_y);
...
CalcNextScrollFromScrollTargetAndClamp(ImGuiWindow *window, bool snap_on_edges) {
    ...
    if (window->ScrollTarget.x < FLT_MAX) {
        ...
        if (snap_on_edges && cr_x <= 0.0f && target_x <= window->WindowPadding.x)
            target_x = 0.0f;
        else if (snap_on_edges && cr_x >= 1.0f && target_x >=
    ...
    if (window->ScrollTarget.y < FLT_MAX) {
        ...
        if (snap_on_edges && cr_y >= 1.0f &&
            target_y >=
            window->ContentSize.y + window->WindowPadding.y + g.Style.ItemSpacing.y)
...
void
ImGui::BeginTooltipEx(ImGuiWindowFlags extra_flags, ImGuiTooltipFlags tooltip_flags) {
    ImGuiContext &g = *GImGui;
...
ImVec2
ImGui::FindBestWindowPosForPopupEx(const ImVec2 &ref_pos, const ImVec2 &size,
                                   ImGuiDir *last_dir,
    ...
    for (int n = (*last_dir != ImGuiDir_None) ? -1 : 0; n < ImGuiDir_COUNT; n++) {
        ...
        if (avail_w < size.x || avail_h < size.y)
            continue;
        ImVec2 pos;
        pos.x = (dir == ImGuiDir_Left) ? r_avoid.Min.x - size.x : (dir == ImGuiDir_Right)
                                                                  ? r_avoid.Max.x
                                                                  : base_pos_clamped.x;
        pos.y = (dir == ImGuiDir_Up) ? r_avoid.Min.y - size.y : (dir == ImGuiDir_Down)
                                                                ? r_avoid.Max.y
                                                                : base_pos_clamped.y;
        *last_dir = dir;
        return pos;
...
ImVec2 ImGui::FindBestWindowPosForPopup(ImGuiWindow *window) {
    ...
    if (window->Flags & ImGuiWindowFlags_Popup) {
        ImRect r_avoid = ImRect(window->Pos.x - 1, window->Pos.y - 1, window->Pos.x + 1,
                                window->Pos.y + 1);
        return FindBestWindowPosForPopupEx(window->Pos, window->Size,
                                           &window->AutoPosLastDirection,
                                           r_outer, r_avoid);
...
static void inline
NavClampRectToVisibleAreaForMoveDir(ImGuiDir move_dir, ImRect &r,
                                    const ImRect &clip_rect) {
    ...
    if (quadrant == g.NavMoveDir) {
        ...
        if (dist_box == result->DistBox) {
            ...
            if (dist_center < result->DistCenter) {
                ...
                if (((g.NavMoveDir == ImGuiDir_Up || g.NavMoveDir == ImGuiDir_Down) ? dby
                                                                                    : dbx) <
...
static void
ImGui::NavProcessItem(ImGuiWindow *window, const ImRect &nav_bb, const ImGuiID id) {
    ImGuiContext &g = *GImGui;
    ...
    if ((g.NavId != id ||
        ...
        ImGuiNavMoveResult *result = (window == g.NavWindow) ? &g.NavMoveResultLocal
...
void
ImGui::NavMoveRequestForward(ImGuiDir move_dir, ImGuiDir clip_dir, const ImRect &bb_rel,
...
ImVec2
ImGui::GetNavInputAmount2d(ImGuiNavDirSourceFlags dir_sources, ImGuiInputReadMode mode,
...
#undef NAV_MAP_KEY
    ...
    if (g.NavWindow && !(g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs) &&
        ...
        ImVec2 scroll_dir = GetNavInputAmount2d(ImGuiNavDirSourceFlags_PadLStick,
                                                ImGuiInputReadMode_Down, 1.0f / 10.0f,
                                                10.0f);
    ...
    ImRect nav_rect_rel = (g.NavWindow &&
...
static void ImGui::NavUpdateMoveResult() {
    ...
    ImGuiNavMoveResult *result = (g.NavMoveResultLocal.ID != 0) ? &g.NavMoveResultLocal
                                                                : &g.NavMoveResultOther;
...
static float ImGui::NavUpdatePageUpPageDown() {
    ...
    const bool page_up_held =
            IsKeyDown(g.IO.KeyMap[ImGuiKey_PageUp]) &&
    ...
    const bool home_pressed =
            IsKeyPressed(g.IO.KeyMap[ImGuiKey_Home]) &&
...
static void ImGui::NavUpdateWindowing() {
    ...
    if (start_windowing_with_gamepad || start_windowing_with_keyboard)
        if (ImGuiWindow *window = g.NavWindow ? g.NavWindow : FindWindowNavFocusable(
    ...
    if (apply_toggle_layer && g.NavWindow) {
        ...
        const ImGuiNavLayer new_nav_layer = (g.NavWindow->DC.NavLayerActiveMask &
                                            ...
                                            ? (ImGuiNavLayer) (
                        (int) g.NavLayer ^ 1) : ImGuiNavLayer_Main;
...
bool
ImGui::SetDragDropPayload(const char *type, const void *data, size_t data_size,
                          ImGuiCond cond) {
    ImGuiContext &g = *GImGui;
    ImGuiPayload &payload = g.DragDropPayload;
    if (cond == 0)
...
static void
WindowSettingsHandler_WriteAll(ImGuiContext *ctx, ImGuiSettingsHandler *handler,
                               ImGuiTextBuffer *buf) {
...
void ImGui::ShowMetricsWindow(bool *p_open) {
    ...
    const char *trt_rects_names[TRT_Count] = {"OuterRect", "WorkRect", "HostClipRect",
                                              "InnerClipRect", "BackgroundClipRect",
                                              "ColumnsRect",
                                              "ColumnsClipRect",
    ...
    struct Funcs {
        static ImRect GetWindowRect(ImGuiWindow *window, int rect_type) {
            if (rect_type == WRT_OuterRect) { return window->Rect(); }
                     ...
                     WRT_OuterRectClipped) { return window->OuterRectClipped; }
                ...
                return ImRect(min, min + window->ContentSize);
                       ...
                       WRT_ContentRegionRect) { return window->ContentRegionRect; }
            IM_ASSERT(0);
            return ImRect();
        }

        static void
        NodeDrawList(ImGuiWindow *window, ImDrawList *draw_list, const char *label) {
            ...
            for (const ImDrawCmd *pcmd = draw_list->CmdBuffer.begin();
                 pcmd <
                 draw_list->CmdBuffer.end(); elem_offset += pcmd->ElemCount, pcmd++) {
                ...
                for (unsigned int base_idx = elem_offset;
                    ...
                    ImVec2 triangle[3];
                    for (int n = 0; n < 3; n++)
                        triangle[n] = draw_list->VtxBuffer[idx_buffer ? idx_buffer[
                                base_idx + n] : (
                                                                   base_idx + n)].pos;
                ...
                while (clipper.Step())
                    for (int prim = clipper.DisplayStart, idx_i =
                        ...
                        for (int n = 0; n < 3; n++, idx_i++) {
                            ImDrawVert &v = draw_list->VtxBuffer[idx_buffer
                                                                 ? idx_buffer[idx_i]
                                                                 : idx_i];
                            triangle[n] = v.pos;
        ...
        static void NodeWindow(ImGuiWindow *window, const char *label) {
            ...
            ImGui::BulletText(
                    ...
                    ? window->BeginOrderWithinContext : -1);
            ImGui::BulletText(
    ...
    if (ImGui::TreeNode("Tools")) {
        ...
        if (show_windows_rects && g.NavWindow) {
            ...
            for (int rect_n = 0; rect_n < WRT_Count; rect_n++) {
                ...
                ImGui::Text("(%6.1f,%6.1f) (%6.1f,%6.1f) Size (%6.1f,%6.1f) %s", r.Min.x,
                            r.Min.y,
