...
namespace aur::geometry_generators {
    ...
    geometry_data_type generate_triangle_geometry_data(float size = 1.0f) {
        ...
        for (int i = 0; i < 3; ++i) {
            float angle{
                    static_cast<float>(i) / 3.0f * 2.0f * static_cast<float>(M_PI) -
    ...
    geometry_data_type
    generate_vector_geometry_data(glm::vec3 position, glm::vec3 vector) {
        std::vector<unsigned int> indices;
        std::vector<Vertex> vertices;
        indices.push_back(0);
        indices.push_back(1);
    ...
    geometry_data_type
    generate_circle_geometry_data(float radius, unsigned int segment_count) {
        std::vector<unsigned int> indices;
        std::vector<Vertex> vertices;
        for (int i = 0; i < segment_count; ++i) {
            float angle{static_cast<float>(i) / static_cast<float>(segment_count) *
    ...
    geometry_data_type generate_plane_geometry_data(float width, float height,
        ...
        for (unsigned int i = 0; i <= height_segments_count; ++i) {
            ...
            float v{static_cast<float>(i) /
                    static_cast<float>(height_segments_count)};
            ...
            for (unsigned int j = 0; j <= width_segments_count; ++j) {
                ...
                float u{static_cast<float>(j) /
                        static_cast<float>(width_segments_count)};
        ...
        //calculate_tangent_space(indices, vertices);

        return std::make_pair(indices, vertices);
    }

    geometry_data_type
    generate_sphere_geometry_data(float radius, unsigned int segment_count,
        ...
        for (unsigned int ring = 0; ring <= ring_count; ++ring) {
            ...
            for (unsigned int segment = 0; segment <= segment_count; ++segment) {
                float u{static_cast<float>(segment) /
