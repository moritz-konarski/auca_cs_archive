...
namespace aur {
        ...
        void update(const Material &material) final {
            ...
            if (_requires_indices_update) {
                ...
                glBufferData(
                        ...
                        _convert_usage_strategy_to_es2_buffer_usage_strategy(
                                _indices_usage_strategy)
                );
            ...
            if (_requires_indices_update || _requires_vertices_update) {
                ...
                GLsizei stride = sizeof(GLfloat) * 24;

                int position_attribute_location{
                        attributes.count("position") > 0 ? attributes.at("position")
                                                         : -1};
                ...
                int color_attribute_location{
                        attributes.count("color") > 0 ? attributes.at("color") : -1};
                ...
                int normal_attribute_location{
                        attributes.count("normal") > 0 ? attributes.at("normal")
                                                       : -1};
                if (normal_attribute_location != -1) {
                    ...
                    glVertexAttribPointer(
                            ...
                            3, GL_FLOAT, GL_FALSE, stride,
                            reinterpret_cast<const GLvoid *>(sizeof(GLfloat) * 7)
                    );
                }

                int texture1_coordinates_attribute_location{
                        attributes.count("texture1_coordinates") > 0 ? attributes.at(
                                "texture1_coordinates") : -1};
                if (texture1_coordinates_attribute_location != -1) {
                    ...
                    glVertexAttribPointer(
                            texture1_coordinates_attribute_location,
                            4, GL_FLOAT, GL_FALSE, stride,
                            reinterpret_cast<const GLvoid *>(sizeof(GLfloat) * 10)
                    );
                }

                int texture2_coordinates_attribute_location{
                        attributes.count("texture2_coordinates") > 0 ? attributes.at(
                                "texture2_coordinates") : -1};
                if (texture2_coordinates_attribute_location != -1) {
                    ...
                    glVertexAttribPointer(
                            texture2_coordinates_attribute_location,
                            4, GL_FLOAT, GL_FALSE, stride,
                            reinterpret_cast<const GLvoid *>(sizeof(GLfloat) * 14)
                    );
                }

                int tangent_attribute_location{
                        attributes.count("tangent") > 0 ? attributes.at(
                                "tangent") : -1};
                if (tangent_attribute_location!= -1) {
                    ...
                    glVertexAttribPointer(
                            tangent_attribute_location,
                            3, GL_FLOAT, GL_FALSE, stride,
                            reinterpret_cast<const GLvoid *>(sizeof(GLfloat) * 18)
                    );
                }

                int bitangent_attribute_location{
                        attributes.count("bitangent") > 0 ? attributes.at(
                                "bitangent") : -1};
                if (bitangent_attribute_location!= -1) {
                    glEnableVertexAttribArray(
                            bitangent_attribute_location);
                    glVertexAttribPointer(
                            bitangent_attribute_location,
                            3, GL_FLOAT, GL_FALSE, stride,
                            reinterpret_cast<const GLvoid *>(sizeof(GLfloat) * 21)
                    );
                }
#ifdef __APPLE__
    ...
    private:
        ...
        static GLenum _convert_usage_strategy_to_es2_buffer_usage_strategy(
                Geometry::UsageStrategy usage_strategy) {
