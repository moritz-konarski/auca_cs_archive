...
namespace aur {
    ...
    public:
        ...
        void render() final {
            ...
            std::sort(std::begin(transparent), std::end(transparent),
                      [&](const auto &a, const auto &b) {
                          return glm::length(camera->get_world_position() -
                                             a->get_world_position()) >
                                 glm::length(camera->get_world_position() -
                                             b->get_world_position());
                      });
            std::sort(std::begin(overlays), std::end(overlays),
                      [](const auto &a, const auto &b) {
                          return a->get_material()->get_overlay_priority() >
                                 b->get_material()->get_overlay_priority();
                      });
    ...
    private:
        void _render_mesh(const std::shared_ptr<Mesh> &mesh) const {
            ...
            glDrawElements(
                    _convert_geometry_type_to_es2_geometry_type(
                            geometry->get_type()),
        ...
        static GLenum
        _convert_geometry_type_to_es2_geometry_type(Geometry::Type type) {
            switch (type) {
