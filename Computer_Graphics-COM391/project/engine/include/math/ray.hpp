...
namespace aur {
    ...
    public:
        Ray(const glm::vec3 &origin, const glm::vec3 &direction) : _origin(origin),
                                                                   _direction(
                                                                           direction) {}
        ...
        [[nodiscard]] intersection_test_result_type
        intersects_with_plane(const Plane &plane) const {
            ...
            if (fabsf(denominator) >= FLT_EPSILON) {
                float numerator =
                        glm::dot(plane.get_normal(), _origin) + plane.get_distance();
        ...
        [[nodiscard]] intersection_test_result_type
        intersects_with_sphere(std::shared_ptr<Sphere> sphere) const {
            bool intersects = false;
            float distance = 0.0f;
