...
namespace aur {
    ...
    public:
        ES2Texture(const std::vector<uint8_t> &image_data, unsigned int width,
                   unsigned int height,
        ...
        void update(unsigned int sampler) final {
            ...
            if (_requires_params_update) {
                ...
                glTexParameteri(
                        ...
                        _convert_filter_type_to_es2_texture_filter_type(
                                _magnification_filter)
                );
                glTexParameteri(
                        GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                        _convert_filter_type_to_es2_texture_filter_type(
                                _minification_filter)
                );
                glTexParameterf(
    ...
    private:
        ...
        static GLenum
        _convert_wrap_mode_to_es2_texture_wrap_mode(Texture::WrapMode wrap_mode) {
            switch (wrap_mode) {
                case Repeat:
                    return GL_REPEAT;
                case MirroredRepeat:
        ...
        static GLenum
        _convert_filter_type_to_es2_texture_filter_type(Texture::FilterType filter) {
            switch (filter) {
                case Nearest:
                    return GL_NEAREST;
                case Linear:
