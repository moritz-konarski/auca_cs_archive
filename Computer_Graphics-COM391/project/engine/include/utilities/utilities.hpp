...
namespace aur::file_utilities {
    ...
    image_data_type read_image_file(const std::string &path) {
        ...
        if (!(bytes_per_pixel == 3 || bytes_per_pixel == 4)) {
            std::cerr
                    << "Invalid image file format (only RGB and RGBA files are supported): '"
                    << path << "'"
                    << std::endl;
            exit(-1);
        }
        ...
        for (int y = surface->h - 1;
             y >= 0; --y) { // flip the image for the OpenGL coordinate system
            for (int x = 0; x < surface->w; ++x) {
                for (int i = 0; i < bytes_per_pixel; ++i) {
                    image_data.push_back(
                            *(data + y * surface->pitch + x * bytes_per_pixel + i));
                }
            }
        }
