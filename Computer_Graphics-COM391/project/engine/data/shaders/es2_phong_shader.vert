...
void main()
{
    ...
    if (tangent != bitangent) {
        vec3 T = normalize(vec3(model_view_matrix * vec4(tangent, 0.0)));
        vec3 B = normalize(vec3(model_view_matrix * vec4(bitangent, 0.0)));
        vec3 N = normalize(vec3(model_view_matrix * vec4(normal, 0.0)));
        fragment_TBN = mat3 (
            T.x, B.x, N.x,
            T.y, B.y, N.y,
            T.z, B.z, N.z
    ...
    if (texture1_enabled) {
        if (texture1_transformation_enabled) {
            ...
            transformed_texture1_coordinates.x += texture1_transformation_matrix[0][3];
    ...
    if (texture2_enabled) {
        if (texture2_transformation_enabled) {
            ...
            transformed_texture2_coordinates.x += texture2_transformation_matrix[0][3];
